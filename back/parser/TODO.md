...
- добавить логгер. Пока бекенда нет и нет логгера, использовать мок реализацию:
  ...
  def foo():
    logger.info("foo")
  ```
- сейчас файл это скрипт. Переделать в модуль (то есть убрать блок main, чтобы файл нельзя было выполнить как скрипт)
- подсказка: для тестирования можно написать простой временный скрипт, который уже будет вызывать парсер
- в файле должна быть одна главная функция, которую будут импортировать в другом месте
- приватные функции начинаются с _ (чтобы их нельзя было импортировать)
